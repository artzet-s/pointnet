...
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: '
                                                       'GPU 0]')
parser.add_argument('--batch', type=int, default=32, help='Batch Size during '
                                                         'training [default: 32]')
        ...
        def train_one_epoch(epoch_num):
            ...
            for j in range(num_batch):
                ...
                (_,
                 ...
                 per_instance_seg_loss_val,
                 label_pred_val,
                 seg_pred_val,
                 pred_seg_res) = sess.run(
                    [train_op,
                     loss,
                     label_loss,
                     seg_loss,
                ...
                total_label_acc += np.mean(np.float32(
                    per_instance_label_pred == cur_labels[begidx: endidx, ...]))
            ...
            (lr_sum,
             ...
             train_seg_loss_sum,
             train_seg_acc_sum) = sess.run(
                [lr_op,
                 bn_decay_op,
                 batch_op,
                 total_train_loss_sum_op,
                 label_train_acc_sum_op,
