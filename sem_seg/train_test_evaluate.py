...
def placeholder_inputs():
    ...
    seg_ph = tf.placeholder(tf.int32, shape=(BATCH_SIZE, NUM_POINT))

    return pointclouds_ph, input_label_ph, labels_ph, seg_ph


def train_tnet(train_data, train_label, test_data, test_label, log, K=9):
	...
	with strategy.scope():
		with tf.Graph().as_default():
			...
			pred, end_points = pointnet_seg.get_model(
				pointclouds_ph,
			...
			# ==================================================================
			# Set loss
			loss = pointnet_seg.get_loss(pred, seg_ph, end_points)
			tf.summary.scalar('loss', loss)

			# ==================================================================
			# Set Accurancy
			correct = tf.equal(tf.argmax(pred, 2), tf.to_int64(seg_ph))
			...
			for epoch in range(MAX_EPOCH + 1):
				...
				train_tnet_one_epoch(train_data,
				                train_label,
				                sess,
				                ops,
				                train_writer,
...
def train_tnet_one_epoch(train_data, train_label, sess, ops, train_writer, log):
	...
	for batch_idx in range(num_batches):
		...
		feed_dict = {
			ops['pointclouds_ph']: current_data[start_idx:end_idx, :, :],
		    ops['seg_ph']: current_label[start_idx: end_idx],
...
def test_tnet_one_epoch(test_data, test_label, sess, ops, test_writer, log):
	""" ops: dict mapping from string to tf ops """
	is_training = False
...
def run_train_test_evaluate():
	...
	test_data, test_label = get_data(
		'/ext_data/artzet_s/test_block_data')
	...
	output_folder = "/ext_data/artzet_s/eval_block_xyz"
	model_path = "/ext_data/artzet_s/log_model_block/model_50.ckpt"
...
def run_train_test_evaluate_light():

	train_data, train_label = get_data(
		'/ext_data/artzet_s/train_light_block_data')

	test_data, test_label = get_data(
		'/ext_data/artzet_s/test_light_block_data')

	log = Log("/ext_data/artzet_s/log_light")
